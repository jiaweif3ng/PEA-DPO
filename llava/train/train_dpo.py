...
def SFT_collator_fn(instances, pad_token_id): # 返回一个 batch dict
    input_ids, labels = tuple([instance[key] for instance in instances]
                              for key in ("input_ids", "labels")) # batch 的大小等于 instances 的长度
...
class DataCollatorForDPODataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        batch = preference_collator_fn(instances, self.tokenizer.pad_token_id) # data_args.dpo_beta
