...
class ModelArguments:
    # model_name_or_path: Optional[str] = field(default="/NAS/fengjw/Project/RePO/base_modals/llava-v1.5-7b") # /NAS/fengjw/Project/RePO/base_modals/llava-v1.5-7b /NAS/fengjw/.cache/base_models/llava-v1.5-7b
    model_name_or_path: Optional[str] = field(default="/NAS/fengjw/.cache/base_models/llava-v1.5-7b") # /NAS/fengjw/Project/RePO/base_modals/llava-v1.5-7b /NAS/fengjw/.cache/base_models/llava-v1.5-7b
    ...
    with torch.inference_mode():
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            for copo_key in ['', 'masked_']:
                ...
                # print(f"per_token_logp: {new_per_token_logp}")
                # print(f"log_logp: {new_log_prob}")
                # print(f"avg_log_logp: {new_average_log_prob}")
                if copo_key == '':
                ...
                # print(f"per_token{new_win_per_token_logp_list}")
                # print(f"logp:{new_win_logp_list}")
                # print(f"avg_logp:{new_win_avg_logp_list}")
            # -------------------------------------------------------- mask image --------------------------------------------------------
...
# # --------------------------------------------------------------------------------------------------------------------------------------
# def get_multimodal_sample_logps_mask(model, dataloader, tokenizer, is_llava15=False): # 编码输入，然后得到 ref model 的对数概率 !!!
#     mask_win_logp_list = []
#     mask_rej_logp_list = []

#     mask_win_avg_logp_list = []
#     mask_rej_avg_logp_list = []

#     mask_win_per_token_logp_list = []
#     mask_rej_per_token_logp_list = []

#     with torch.inference_mode():
#         idx=0
#         for batch in tqdm.tqdm(dataloader):
#             for copo_key in ['', 'mask_']:
#                 input_ids = batch[f'win_input_ids'].cuda()
#                 labels = batch[f'win_labels'].cuda()
#                 attention_mask = batch[f'win_attention_mask'].cuda()
#                 if is_llava15: # True
#                     # print("is llava15")
#                     (
#                         _,
...
#                         images=batch[f'{copo_key}images'].to(dtype=torch.bfloat16, device='cuda'),
#                     )
#                 mask_per_token_logp, mask_log_prob, mask_average_log_prob = get_batch_logps(output.logits, labels, return_all=True)
#                 assert mask_per_token_logp.size(1) >= input_ids.size(1) - 1
#                 mask_per_token_logp = mask_per_token_logp.tolist()
#                 mask_log_prob = mask_log_prob.tolist()
#                 mask_average_log_prob = mask_average_log_prob.tolist()

#                 if copo_key == '':
#                     mask_win_logp_list += mask_log_prob
#                     mask_win_avg_logp_list += mask_average_log_prob
#                     mask_win_per_token_logp_list += mask_per_token_logp
#                 else:
#                     mask_rej_logp_list += mask_log_prob
#                     mask_rej_avg_logp_list += mask_average_log_prob
#                     mask_rej_per_token_logp_list += mask_per_token_logp
#             # print(f'{key} logits in {output.logits.shape}, logp in {log_prob.shape} avg_logp in {average_log_prob.shape}', flush=True)

#     return mask_win_logp_list, mask_win_avg_logp_list, mask_win_per_token_logp_list, mask_rej_logp_list, mask_rej_avg_logp_list, mask_rej_per_token_logp_list
# # --------------------------------------------------------------------------------------------------------------------------------------
def write_logp_to_preference_parquet(origin_data, cache_file, logps, new_logps, overwrite_logps=False):
    ...
    for index in range(len(logps)): # logps list[tuple]
        ...
        new_logp_data['new_logps']=new_logps[index] # 5.7 add
        ...
        if 'logps' in new_line.keys():
            ...
            new_line['new_logps'] = json.dumps(new_logp_data)
        else:
    ...
    # # ---------------------------------------- 5.7 ----------------------------------------
    # for index in range(len(new_logps)): # logps list[tuple]
    #     line = origin_data[index]
    #     new_logp_data = {} # 5.7 add
    #     new_logp_data['new_logps']=new_logps[index] # 5.7 add
    #     new_line = copy.deepcopy(line)
