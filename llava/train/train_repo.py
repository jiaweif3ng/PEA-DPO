...
#     concatenated_input_ids = concate_pad(win_batch['input_ids'], rej_batch['input_ids'], pad_token_id) # 将 win_batch 和 rej_batch 中的所有 input_ids 填充到相同的长度，然后放到同一个列表中
#     concatenated_labels = concate_pad(win_batch['labels'], rej_batch['labels'], -100)
#     concatenated_attention_mask = concatenated_input_ids.ne(pad_token_id) # 对 concatenated_input_ids 中的 pad_token 生成掩码
...
# #     this_logits = this_logits.to(torch.float32) # beta in paper
...
# def repo_loss(policy_chosen_logps: torch.FloatTensor,
#              policy_rejected_logps: torch.FloatTensor,
#              reference_chosen_logps: torch.FloatTensor,
#              reference_rejected_logps: torch.FloatTensor,
#              beta: float,
...
#     # losses = -F.logsigmoid(beta * logits)
#     losses = torch.relu(8.0 - beta * logits) # ReLU DPO
#     chosen_rewards = beta * (policy_chosen_logps -
#                              reference_chosen_logps).detach()
#     rejected_rewards = beta * \
#         (policy_rejected_logps - reference_rejected_logps).detach()
...
#         data_dict = inputs # 包含 data 和概率对数
#         policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta = self.get_beta_and_logps(
#             data_dict, model, self.args, is_llava15=True) # del vlm_win_logits, vlm_rej_logits

#         losses, chosen_rewards, rejected_rewards = repo_loss(policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta=beta)
...
#                                              ref_rej_logp, ref_win_logp, reward_accuracies, beta,
#                                              gather_and_do_mean) # del vlm_win_logits, vlm_rej_logits
#         self.log(metrics)
#         return loss
...
#                                ref_rej_logp, ref_win_logp, reward_accuracies, beta, 
#                                preprocess_func,
#                                ): # del vlm_win_logits, vlm_rej_logits
#         t = task
#         metrics = {}
...
#             )
#             # print(f"embeds type: {type(concatenated_inputs_embeds)}") ???
...
#                 output.logits, concatenated_labels, return_per_token_logp=False) # 计算对数概率
...
#         policy_win_logp, policy_rej_logp = concatenated_logp.split( # log_prob
#             [win_size, rej_size])
#         return policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta # del vlm_win_logits, vlm_rej_logits
...
class DPODataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        #     source, self.tokenizer, self.multimodal_cfg, preprocess_func=preprocess_func)
        # ---------------------------------------- 5.7 ----------------------------------------
        rej_data_dict, win_data_dict, new_rej_data_dict, new_win_data_dict = encode_multimodal_preference_sample(
            source, self.tokenizer, self.multimodal_cfg, preprocess_func=preprocess_func)
        # ---------------------------------------- 5.7 ----------------------------------------
        # return rej_data_dict, win_data_dict # 返回两个字典分别包含：input_ids, labels, image,+ 3(ref logps)
...
def preference_collator_fn(instances, pad_token_id):
    ...
    # ---------------------------------------- 5.7 ----------------------------------------
    new_concatenated_input_ids = concate_pad(new_win_batch['input_ids'], new_rej_batch['input_ids'], pad_token_id) # 将 win_batch 和 rej_batch 中的所有 input_ids 填充到相同的长度，然后放到同一个列表中
    new_concatenated_labels = concate_pad(new_win_batch['labels'], new_rej_batch['labels'], -100)
    new_concatenated_attention_mask = new_concatenated_input_ids.ne(pad_token_id) # 对 concatenated_input_ids 中的 pad_token 生成掩码
    # ---------------------------------------- 5.7 ----------------------------------------
...
def repo_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    losses = -F.logsigmoid(beta * logits)
    # losses = torch.relu(6.0 - beta * logits) # ReLU DPO
    chosen_rewards = beta * (policy_chosen_logps -
                             reference_chosen_logps).detach()
    rejected_rewards = beta * \
        (policy_rejected_logps - reference_rejected_logps).detach()
...
def copo_repo_loss(policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor,
             beta: float,
    ...
    losses = -F.logsigmoid(beta * logits)
    # losses = torch.relu(15.0 - beta * logits) # ReLU DPO
    chosen_rewards = beta * (policy_chosen_logps -
                             reference_chosen_logps).detach()
    rejected_rewards = beta * \
        (policy_rejected_logps - reference_rejected_logps).detach()
...
class LLaVA15DPOTrainer(Trainer):
    ...
    def compute_loss(self, model: Module, inputs: dict, return_outputs=False):
        ...
        # ---------------------------------------- 5.8 ----------------------------------------
        policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, new_policy_win_logp, new_policy_rej_logp, new_ref_win_logp, new_ref_rej_logp, beta = self.get_beta_and_logps(
            data_dict, model, self.args, is_llava15=True)
        # ---------------------------------------- 5.8 ----------------------------------------

        # losses, chosen_rewards, rejected_rewards = repo_loss(policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta=beta)
        losses1, chosen_rewards1, rejected_rewards1 = repo_loss(policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta=beta)

        # ---------------------------------------- 5.8 ----------------------------------------
        # losses2, chosen_rewards2, rejected_rewards2 = copo_repo_loss(new_policy_win_logp, new_policy_rej_logp, new_ref_win_logp, new_ref_rej_logp, beta=beta)
        losses2, chosen_rewards2, rejected_rewards2 = repo_loss(new_policy_win_logp, new_policy_rej_logp, new_ref_win_logp, new_ref_rej_logp, beta=beta)
        # ---------------------------------------- 5.8 ----------------------------------------
        ...
        # loss = losses1.mean()
        # loss = losses2.mean()
        loss = losses1.mean() + 0.7 * losses2.mean()
    ...
    # ---------------------------------------- 5.8 ----------------------------------------
    def collect_preference_metrics(self, metrics, task,
                               chosen_rewards1, rejected_rewards1,
                               policy_rej_logp, policy_win_logp,
                               ref_rej_logp, ref_win_logp, reward_accuracies1, 
                               chosen_rewards2, rejected_rewards2,
                               new_policy_rej_logp, new_policy_win_logp,
    ...
    # ---------------------------------------- 5.8 ----------------------------------------
        t = task
        metrics = {}
        # gap_local1 = (policy_win_logp - policy_rej_logp - ref_win_logp + ref_rej_logp).detach()
        # gap_local1 = all_gather_if_needed(gap_local1, self.rank, self.world_size)
        # loss_local1 = -F.logsigmoid(beta * gap_local1)

        # gap_local2 = (new_policy_win_logp - new_policy_rej_logp - new_ref_win_logp + new_ref_rej_logp).detach()
        # gap_local2 = all_gather_if_needed(gap_local2, self.rank, self.world_size)
        # loss_local2 = -F.logsigmoid(beta * gap_local2)
        ...
        # ---------------------------------------- 5.8 ----------------------------------------
        metrics[f'rewards_{t}/chosen1'] = preprocess_func(chosen_rewards1)
        metrics[f'rewards_{t}/rejected1'] = preprocess_func(rejected_rewards1)
        metrics[f'logps_{t}/rejected1'] = preprocess_func(policy_rej_logp)
        metrics[f'logps_{t}/chosen1'] = preprocess_func(policy_win_logp)
        metrics[f'logps_{t}/ref_rejected1'] = preprocess_func(ref_rej_logp)
        metrics[f'logps_{t}/ref_chosen1'] = preprocess_func(ref_win_logp)

        metrics[f'rewards_{t}/chosen2'] = preprocess_func(chosen_rewards2)
        metrics[f'rewards_{t}/rejected2'] = preprocess_func(rejected_rewards2)
        metrics[f'logps_{t}/rejected2'] = preprocess_func(new_policy_rej_logp)
        metrics[f'logps_{t}/chosen2'] = preprocess_func(new_policy_win_logp)
        metrics[f'logps_{t}/ref_rejected2'] = preprocess_func(new_ref_rej_logp)
        metrics[f'logps_{t}/ref_chosen2'] = preprocess_func(new_ref_win_logp)
        # ---------------------------------------- 5.8 ----------------------------------------
        # metrics[f'logps_{t}/vlm_rejected'] = preprocess_func(vlm_rej_logits)
        # metrics[f'logps_{t}/vlm_chosen'] = preprocess_func(vlm_win_logits)
        # metrics[f'logps_{t}/vlm_diff_sigmoid'] = logits_diff.cpu().numpy().tolist()
        ...
        #     metrics[f'rewards_{t}/rejected']
        # ---------------------------------------- 5.8 ----------------------------------------
        metrics[f'rewards_{t}/accuracies1'] = preprocess_func(reward_accuracies1)
        metrics[f'rewards_{t}/accuracies2'] = preprocess_func(reward_accuracies2)
        ...
        # metrics[f'loss1'] = loss_local1 # 5.17 add
        # metrics[f'loss2'] = loss_local2 # 5.17 add
        # ---------------------------------------- 5.8 ----------------------------------------
        return metrics
    ...
    def get_beta_and_logps(self, data_dict, model, args, is_minicpm=False, is_llava15=False):
        ...
        if args.task == 'DPO': # default: true
            ...
            if is_minicpm: # default: false
                ...
                concatenated_images = torch.cat([images, images], dim=0)
                # ---------------------------------------- 5.7 ----------------------------------------
                new_concatenated_images = torch.cat([images, masked_images], dim=0) # ?????? yes
                # ---------------------------------------- 5.7 ----------------------------------------
        elif args.task == 'KTO':
            win_images = data_dict.pop('win_images')
            rej_images = data_dict.pop('rej_images')
        ...
        if is_llava15: # default: true
            ...
            new_log_prob, new_average_log_prob = get_batch_logps(
                new_output.logits, new_concatenated_labels, return_per_token_logp=False) # 计算对数概率
        ...
        policy_win_logp, policy_rej_logp = concatenated_logp.split( # log_prob
            [win_size, rej_size])
        # ---------------------------------------- 5.8 ----------------------------------------
        new_policy_win_logp, new_policy_rej_logp = new_concatenated_logp.split( # log_prob
            [new_win_size, new_rej_size])
        # ---------------------------------------- 5.8 ----------------------------------------
        # return policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, beta # del vlm_win_logits, vlm_rej_logits
        # ---------------------------------------- 5.8 ----------------------------------------
        return policy_win_logp, policy_rej_logp, ref_win_logp, ref_rej_logp, new_policy_win_logp, new_policy_rej_logp, new_ref_win_logp, new_ref_rej_logp, beta
        # ---------------------------------------- 5.8 ----------------------------------------
