...
def dpo_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    if loss_type == 'dpo':
        ...
        losses = torch.relu(gamma - beta * logits)
    else:
        raise ValueError(
...
def dpo_loss1(policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
             reference_rejected_logps: torch.FloatTensor,
             beta: float,
    ...
    if loss_type == 'dpo':
        ...
        losses = torch.relu(beta * logits - gamma)
    else:
        raise ValueError(
            f"Unknown loss type: {loss_type}"
        )
...
class LLaVA15DPOTrainer(Trainer):
    ...
    def compute_loss(self, model: Module, inputs: dict, return_outputs=False):
        ...
        if self.CoPO:
            ...
            loss = losses1.mean() + self.CoPO_coef * losses2.mean()
